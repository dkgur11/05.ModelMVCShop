...
public class Page {
	...
	public Page( int currentPage, int totalCount,	int pageUnit, int pageSize ) {
		this.totalCount = totalCount;//21
		this.pageUnit = pageUnit;//5
		this.pageSize = pageSize;//3
		
		this.maxPage = (pageSize == 0) ? totalCount :  (totalCount-1)/pageSize +1; //7
		this.currentPage = ( currentPage > maxPage) ? maxPage : currentPage;//1
		
		this.beginUnitPage = ( (currentPage-1) / pageUnit ) * pageUnit +1 ;//1
		
		if( maxPage <= pageUnit ){
			this.endUnitPage = maxPage;
		}else{
			this.endUnitPage = beginUnitPage + (pageUnit -1);//5
